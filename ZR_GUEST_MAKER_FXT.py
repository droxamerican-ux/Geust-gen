...
def generate_random_name():
    # বিশেষ কারাক্টার (superscript numbers)
    special_chars = "⠔⠓⠒⠑⠐"
    
    # ৩টি র্যান্ডম বিশেষ কারাক্টার নির্বাচন করুন
...
def GET_PAYLOAD_BY_DATA(JWT_TOKEN, NEW_ACCESS_TOKEN, date,response , status_code , name, uid, password, region):
        try:
            ...
            # FIXED: Extract account_id from field 1 instead of field 2
            account_id = None
            if data and '1' in data and 'data' in data['1']:
...
# Load existing accounts from JSON file if exists
accounts = []
json_filename = f'accounts_{region}.json'
...
if os.path.exists(json_filename):
    try:
        ...
        print(f"{lg}{bold}Loaded {len(accounts)} existing accounts from {json_filename}")
    except Exception as e:
        print(f"{red}{bold}Error loading existing accounts: {e}")
...
for i in range(100000):
    try:
        ...
        if status_code == 200:
            ...
            # Save to JSON file after each successful creation
            with open(json_filename, 'w', encoding='utf-8') as f:
                json.dump(accounts, f, indent=2, ensure_ascii=False)
            
            print(f"{lg}{bold}Success: {uid}|{password} | Account ID: {account_id} | Status: {status_code}")
            print(f"{green}{bold}Total accounts created: {len(accounts)}")
...
print(f"{lg}{bold}Finished! Total accounts created: {len(accounts)}")
print(f"{lg}{bold}Accounts saved to: {json_filename}")
